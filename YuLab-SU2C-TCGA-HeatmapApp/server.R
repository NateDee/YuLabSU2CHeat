	...
	drawUserHeatmap <- function() {
		...
		if (doInputHeat() == "Yes") {
			...
			returnHeat <- function(df) {
				...
				#Run Heatmap:
				heatmap.2(mat, scale = "none", trace="none", dendrogram=dendrogram, breaks=seq(scalelow, scalehigh, length.out=101), Rowv=Rowv, Colv=FALSE, col=palette, ColSideColors=group, keysize=0.75, key.par = list(cex=0.5), labCol=FALSE, cexRow=cexRow)
			}
